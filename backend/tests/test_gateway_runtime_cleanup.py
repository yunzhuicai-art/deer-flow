...
def test_frontend_proxies_gateway_api_with_next_rewrites():
    next_config = _read("frontend/next.config.js")
    api_client = _read("frontend/src/core/api/api-client.ts")

    assert "async rewrites()" in next_config
    assert "DEER_FLOW_INTERNAL_GATEWAY_BASE_URL" in next_config
    assert 'source: "/api/langgraph/:path*"' in next_config
    assert "destination: `${gatewayURL}/api/:path*`" in next_config
